...
import { Plus, Film, Trash2, ArrowRight, Star, Info, Users, Sparkles } from 'lucide-react';
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 max-w-lg">
            Suche nach deinen Wunschfilmen für heute Abend. Jeder Vorschlag landet direkt im gemeinsamen Abstimmungspool.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          >
            ...
            <span>{canAddMore ? `Film suchen (${myMovies.length}/${maxLimit})` : `Limit erreicht (${maxLimit}/${maxLimit})`}</span>
          </button>
        ...
        {room.movies.length === 0 ? (
          <div className="glass-panel rounded-3xl p-12 text-center border border-white/5">
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1 max-w-sm mx-auto">
              Klicke oben auf <strong>"Film suchen"</strong>, um den ersten Film für die Runde hinzuzufügen!
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {room.movies.map((movie) => {
              ...
              return (
                ...
                >
                  <div className="relative h-48 bg-theater-950 overflow-hidden cursor-pointer" onClick={() => setSelectedDetailMovie(movie)}>
                    <img
                      src={movie.poster}
                      alt={movie.title}
                    ...
                    {movie.suggestedBy && (
                      <div className="absolute bottom-3 left-3 flex items-center gap-1.5 bg-theater-950/85 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/10 text-[11px] text-slate-200">
                        ...
                        <span className="font-medium truncate max-w-[120px]">{movie.suggestedBy.name}</span>
                      </div>
                    )}
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {movie.genre.slice(0, 3).map((g) => (
                          <span
                            key={g}
                            className="text-[10px] font-medium bg-theater-800 text-slate-300 px-2 py-0.5 rounded"
                          >
                            {g}
                          </span>
                        ))}
