...
// Curated high-rated movies collection with rich metadata, genres, exact runtime & soundtrack highlights
...
export async function searchMovies(params: MovieSearchParams | string): Promise<SearchMovieResult[]> {
  const options: MovieSearchParams = typeof params === 'string' ? { query: params } : params;
  const trimmed = (options.query || '').trim();
  const genreFilter = options.genre && options.genre !== 'all' ? options.genre.toLowerCase() : null;
  ...
  // Try TMDB API if a query is provided
  if (trimmed) {
    const tmdbApiKey = process.env.TMDB_API_KEY || "15d2ea6d0dc1d476efbca3eba2b9bbfb";
    try {
      ...
      if (response.ok) {
        ...
        if (data.results && data.results.length > 0) {
          results = data.results.slice(0, 20).map((item: any) => {
            ...
            // Check if movie title/id matches our curated soundtrack highlights
  ...
  // If no search query or TMDB returned nothing, start with curated list
  if (results.length === 0) {
    if (trimmed) {
  ...
  filtered.sort((a, b) => {
    ...
    // Default: popularity / curated order with soundtrack highlights boosted if soundtrack filter active
    if (soundtrackOnly) {
